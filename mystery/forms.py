...
class InterestForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['meet_choice'] = forms.TypedChoiceField(choices=choices, widget=forms.RadioSelect)
        self.fields['departments'].initial = self.fields['departments'].queryset.all()
    ...
    def is_valid(self):
        ...
        if is_valid and len(self.cleaned_data['locations']) == 0 and \
           self.cleaned_data['meet_choice'] != Interest.CHOICE_VIDEO:
            self._errors['locations'] = ErrorList([u"This field is required unless Video Chat is selected"])
            is_valid = False
